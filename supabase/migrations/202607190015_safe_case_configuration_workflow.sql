...
begin
  ...
  if target_case.intake_status <> 'link_ready'
     or exists (
       select 1 from public.case_intake_responses response
       where response.case_id = p_case_id
     ) then
    raise exception using errcode = 'P0001', message = 'configuration_not_editable';
  end if;
  if exists (
    select 1
  ...
  from jsonb_array_elements_text(p_payload->'moduleIds')
  ...
  insert into public.case_prefilled_fields (
    case_id, field_key, field_value, source_type, source_note, customer_can_edit
  )
  select
    p_case_id,
    fact."fieldKey",
  ...
  where existing.case_id = p_case_id
    ...
    and not exists (
      ...
      from jsonb_array_elements(
        coalesce(p_payload->'profileCandidates', '[]'::jsonb)
      ) candidate_item
  ...
  loop
    ...
    if candidate_id is not null and exists (
      ...
      set maps_url = nullif(candidate->>'mapsUrl', ''),
          ...
          map_pin_notes = nullif(candidate->>'mapPinNotes', ''),
          ...
          rating = nullif(candidate->>'rating', '')::numeric,
          review_count = nullif(candidate->>'reviewCount', '')::integer,
          possible_creator = nullif(candidate->>'possibleCreator', ''),
          customer_controls_profile = nullif(candidate->>'customerControlsProfile', ''),
          ownership_request_status = nullif(candidate->>'ownershipRequestStatus', ''),
          relation_notes = nullif(candidate->>'relationNotes', ''),
          independent_business_signals = coalesce(
            candidate->'independentBusinessSignals', '{}'::jsonb
          )
      where id = candidate_id;
    else
      insert into public.current_profile_candidates (
        ...
        nullif(candidate->>'mapPinNotes', ''),
        nullif(candidate->>'displayedPhone', ''),
        nullif(candidate->>'displayedWebsite', ''),
        nullif(candidate->>'displayedCategory', ''),
        nullif(candidate->>'rating', '')::numeric,
        nullif(candidate->>'reviewCount', '')::integer,
        nullif(candidate->>'possibleCreator', ''),
        nullif(candidate->>'customerControlsProfile', ''),
        nullif(candidate->>'ownershipRequestStatus', ''),
        nullif(candidate->>'relationNotes', ''),
        coalesce(candidate->'independentBusinessSignals', '{}'::jsonb),
  ...
  with ordered as (
    select
      id,
      row_number() over (
        order by array_position(retained_candidate_ids, id)
      ) - 1 as new_order
    from public.current_profile_candidates
  ...
  select
    ...
    coalesce(item->'conditionalLogic', '{}'::jsonb),
    ordinality::integer - 1
  from jsonb_array_elements(
    coalesce(p_payload->'customQuestions', '[]'::jsonb)
  ...
  insert into public.case_requested_evidence (
    case_id, evidence_category, label, help_text, required, sort_order
  )
  select
    p_case_id,
    ...
    ordinality::integer - 1
  from jsonb_array_elements(
    coalesce(p_payload->'requestedEvidence', '[]'::jsonb)
  ) with ordinality evidence(item, ordinality);

  insert into public.case_activity_log (
    case_id, actor_type, actor_id, action, metadata
  ) values (
    p_case_id,
    'admin',
    p_actor_id,
    'case_setup_updated',
    jsonb_build_object(
      'changed_sections', jsonb_build_array(
        'case_details',
        'question_modules',
        'prefilled_fields',
        'profile_candidates',
...
revoke all on function public.update_case_configuration(uuid, jsonb, uuid)
...
begin
  if jsonb_typeof(p_payload) <> 'object'
     or not exists (select 1 from public.admin_profiles where user_id = p_actor_id) then
    raise exception using errcode = '42501', message = 'invalid_case_clone';
  end if;

  select * into source_case
  from public.cases
  ...
  if source_case.intake_status <> 'link_ready'
     or exists (
       select 1 from public.case_intake_responses response
       where response.case_id = p_source_case_id
     ) then
    raise exception using errcode = 'P0001', message = 'configuration_not_cloneable';
  end if;

  new_case_id := public.create_case_with_configuration(
    (p_payload - 'sourceCaseId') || jsonb_build_object('actorId', p_actor_id)
  );

  update public.current_profile_candidates profile
  set map_pin_notes = nullif(candidate.item->>'mapPinNotes', ''),
      rating = nullif(candidate.item->>'rating', '')::numeric,
      review_count = nullif(candidate.item->>'reviewCount', '')::integer,
      possible_creator = nullif(candidate.item->>'possibleCreator', ''),
      customer_controls_profile = nullif(candidate.item->>'customerControlsProfile', ''),
      ownership_request_status = nullif(candidate.item->>'ownershipRequestStatus', ''),
      independent_business_signals = coalesce(
        candidate.item->'independentBusinessSignals', '{}'::jsonb
      ),
      customer_raw_response = candidate.item - 'existingId'
  from jsonb_array_elements(
    coalesce(p_payload->'profileCandidates', '[]'::jsonb)
  ) with ordinality candidate(item, ordinality)
  where profile.case_id = new_case_id
    and profile.customer_client_id is null
    and profile.sort_order = candidate.ordinality::integer - 1;
...
revoke all on function public.clone_case_with_configuration(uuid, jsonb, uuid)
from public, anon, authenticated;
grant execute on function public.clone_case_with_configuration(uuid, jsonb, uuid)
to service_role;

comment on function public.clone_case_with_configuration(uuid, jsonb, uuid)
is 'Clones validated administrator configuration into a new pre-submission case after atomically locking and validating the source case. Customer responses and operational records are never copied.';
